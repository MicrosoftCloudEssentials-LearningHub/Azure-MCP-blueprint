...
AGENTS: dict[str, dict[str, Any]] = {
    "Claims Intake": {
        "description": "New claims and basic triage",
        "keywords": ["filed", "intake", "new claim", "incident"],
    },
    "Adjuster": {
        "description": "Claim review, estimates, documentation",
        "keywords": ["adjuster", "estimate", "documentation", "photos"],
    },
    "Fraud Investigator": {
        "description": "Fraud risk and red flags",
        "keywords": ["fraud", "risk", "red flag", "investigation"],
    },
    "Settlement Coordinator": {
        "description": "Settlements, approvals, denials",
        "keywords": ["settled", "approved", "denied", "appeal"],
    },
}

DEMO_QUERIES: list[str] = [
    "High-risk fraud claims with fraud score > 0.8",
    "Pending claims awaiting review/approval",
    "High-value claims over $50,000",
...
def _choose_model_for_task(query: str) -> str:
    q = query.lower()
    if len(q) > 120 or any(k in q for k in ["fraud", "analyze", "investigation"]):
        return "gpt-4o"
    return "gpt-4o-mini"


def _route_with_openai(session: requests.Session, endpoint: str, query: str, agent_names: list[str]) -> str | None:
    system = (
        "You are an intent router for an Insurance claims assistant. "
    ...
    return agent if agent in agent_names else None
...
def _summarize_with_openai(
    ...
    system = (
        "You are an insurance claims assistant. Summarize MCP search results concisely. "
        "Do not invent claim IDs. Highlight risk flags and recommended next checks."
    )
    user = json.dumps({"industry": INDUSTRY_NAME, "agent": agent_name, "query": query, "result": raw_result.get("content")})
    resp = _execute_tool(
        session,
        endpoint,
        "openai_chat_completion",
...
def _run_query(session: requests.Session, endpoint: str, tools: set[str], query: str, raw: bool) -> None:
    ...
    chosen = _route_with_openai(session, endpoint, query, agent_names) if "openai_chat_completion" in tools else None
    if not chosen:
...
def main() -> None:
    load_dotenv()
    parser = argparse.ArgumentParser(description=f"{INDUSTRY_NAME} Claims Agent (MCP HTTP)")
