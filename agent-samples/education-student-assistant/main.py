...
AGENTS: dict[str, dict[str, Any]] = {
    "Records Clerk": {
        "description": "Student records and enrollment queries",
        "keywords": ["student", "enroll", "records", "id"],
    },
    "Academic Advisor": {
        "description": "Graduation readiness and course planning",
        "keywords": ["credits", "graduation", "advisor", "plan"],
    },
    "Risk Monitor": {
        "description": "At-risk students and attendance/probation",
        "keywords": ["at-risk", "attendance", "probation", "absences"],
    },
}

DEMO_QUERIES: list[str] = [
    "Honor students with GPA >= 3.5",
    "At-risk students with low attendance or probation",
    "Near graduation students within 15 credits",
    "Students by major (Computer Science)",
...
def _choose_model_for_task(query: str) -> str:
    q = query.lower()
    if len(q) > 120 or any(k in q for k in ["recommend", "plan", "analyze"]):
        return "gpt-4o"
    return "gpt-4o-mini"
...
def _route_with_openai(
    ...
    system = (
        "You are an intent router for an Education assistant. "
...
def _summarize_with_openai(
    ...
    system = (
        "You are an education assistant. Summarize MCP search results concisely. "
        "If the query asks for individual student details, avoid inventing identifiers."
    )
    user = json.dumps(
        {"industry": INDUSTRY_NAME, "agent": agent_name, "query": query, "result": raw_result.get("content")},
        ensure_ascii=False,
    )
    resp = _execute_tool(
        session,
...
def main() -> None:
    load_dotenv()
    parser = argparse.ArgumentParser(description=f"{INDUSTRY_NAME} Student Assistant (MCP HTTP)")
