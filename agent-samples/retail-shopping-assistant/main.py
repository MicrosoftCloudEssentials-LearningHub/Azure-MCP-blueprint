...
This sample intentionally keeps dependencies minimal (requests + dotenv).
If the MCP server exposes `openai_chat_completion`, the assistant will use it for:
  - selecting a specialist role
  - summarizing results
Otherwise it falls back to simple keyword routing and prints raw results.
"""
...
AGENTS: dict[str, dict[str, Any]] = {
    "Shopper Agent": {
        "description": "General customer questions and order lookups",
        "keywords": ["order", "purchase", "history", "customer"],
    },
    "Product Specialist": {
        "description": "Product details, categories, comparisons",
        "keywords": ["product", "sku", "category", "electronics", "compare"],
    },
    "Inventory Manager": {
        "description": "Stock / availability / store location",
        "keywords": ["inventory", "stock", "available", "store", "location"],
    },
    "Loyalty Manager": {
        "description": "Loyalty tiers, points, benefits",
        "keywords": ["loyalty", "points", "tier", "gold", "platinum"],
    },
    "Cart Manager": {
        "description": "Cart-related flows (promos/discounts in this sample)",
        "keywords": ["cart", "discount", "promo", "coupon"],
    },
    "Recommendation Engine": {
        "description": "Gift ideas and recommendations",
        "keywords": ["recommend", "suggest", "gift"],
    },
}

DEMO_QUERIES: list[str] = [
    "Find high-value transactions over $500",
    "Search electronics purchases",
    "Find customer purchase history for loyalty members",
...
def _choose_model_for_task(query: str) -> str:
    q = query.lower()
    if len(q) > 120 or any(k in q for k in ["analyze", "comparison", "recommend", "insights"]):
        return "gpt-4o"
    return "gpt-4o-mini"
...
def _route_with_openai(
    ...
    system = (
        "You are an intent router for a Retail assistant. "
...
def _summarize_with_openai(
    ...
    system = (
        "You are a retail operations assistant. Summarize MCP search results concisely. "
        "If there are no results, say so and suggest a refined query."
    )
    user = json.dumps(
        {
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description=f"{INDUSTRY_NAME} Shopping Assistant (MCP HTTP)")
