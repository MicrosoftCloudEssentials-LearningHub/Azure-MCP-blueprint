...
AGENTS: dict[str, dict[str, Any]] = {
    "Front Desk": {
        "description": "Check-in/check-out and reservation lookups",
        "keywords": ["check-in", "check out", "reservation", "booking"],
    },
    "Loyalty Desk": {
        "description": "VIP/loyalty tier questions",
        "keywords": ["vip", "loyalty", "platinum", "points"],
    },
    "Concierge": {
        "description": "Special requests and service optimization",
        "keywords": ["request", "accessibility", "concierge", "spa", "restaurant"],
    },
}
...
def _choose_model_for_task(query: str) -> str:
    q = query.lower()
    if len(q) > 120 or any(k in q for k in ["recommend", "plan", "itinerary"]):
        return "gpt-4o"
    return "gpt-4o-mini"
...
def _route_with_openai(session: requests.Session, endpoint: str, query: str, agent_names: list[str]) -> str | None:
    system = (
        "You are an intent router for a hospitality concierge assistant. "
...
def _summarize_with_openai(session: requests.Session, endpoint: str, query: str, agent_name: str, raw_result: dict[str, Any]) -> str | None:
    system = (
        "You are a hospitality concierge. Summarize MCP search results concisely. "
        "Highlight arrivals, VIPs, special requests, and follow-ups."
    )
...
def main() -> None:
    load_dotenv()
    parser = argparse.ArgumentParser(description=f"{INDUSTRY_NAME} Concierge (MCP HTTP)")
