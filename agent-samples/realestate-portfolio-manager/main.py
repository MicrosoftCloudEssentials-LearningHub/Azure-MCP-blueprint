...
AGENTS: dict[str, dict[str, Any]] = {
    "Listings Analyst": {
        "description": "Listings and availability queries",
        "keywords": ["listing", "active", "available", "market"],
    },
    "Valuation Analyst": {
        "description": "Pricing and investment signals",
        "keywords": ["price", "valuation", "sq ft", "investment"],
    },
    "Buyer Matching": {
        "description": "Recommendations and buyer fit",
        "keywords": ["recommend", "match", "family", "bedroom"],
    },
}

DEMO_QUERIES: list[str] = [
    "Luxury properties over $1,000,000",
...
def _choose_model_for_task(query: str) -> str:
    q = query.lower()
    if len(q) > 120 or any(k in q for k in ["analyze", "investment", "recommend"]):
        return "gpt-4o"
    return "gpt-4o-mini"
...
def _route_with_openai(session: requests.Session, endpoint: str, query: str, agent_names: list[str]) -> str | None:
    system = (
        "You are an intent router for a Real Estate assistant. "
...
def _summarize_with_openai(session: requests.Session, endpoint: str, query: str, agent_name: str, raw_result: dict[str, Any]) -> str | None:
    system = (
        "You are a real estate portfolio assistant. Summarize MCP search results concisely. "
        "Highlight price, location cues, and suggested next queries."
    )
...
def main() -> None:
    load_dotenv()
    parser = argparse.ArgumentParser(description=f"{INDUSTRY_NAME} Portfolio Manager (MCP HTTP)")
