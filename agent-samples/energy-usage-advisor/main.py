...
AGENTS: dict[str, dict[str, Any]] = {
    "Grid Monitor": {
        "description": "Outages and smart grid alerts",
        "keywords": ["outage", "alert", "grid", "maintenance"],
    },
    "Usage Analyst": {
        "description": "Consumption and peak demand analysis",
        "keywords": ["consumption", "kwh", "peak", "demand"],
    },
    "Renewables Advisor": {
        "description": "Solar / net metering / carbon offset",
        "keywords": ["solar", "renewable", "net metering", "carbon"],
    },
}
...
def _choose_model_for_task(query: str) -> str:
    q = query.lower()
    if len(q) > 120 or any(k in q for k in ["optimize", "analyze", "forecast"]):
        return "gpt-4o"
    return "gpt-4o-mini"
...
def _route_with_openai(session: requests.Session, endpoint: str, query: str, agent_names: list[str]) -> str | None:
    system = (
        "You are an intent router for an Energy usage assistant. "
...
def _summarize_with_openai(session: requests.Session, endpoint: str, query: str, agent_name: str, raw_result: dict[str, Any]) -> str | None:
    system = (
        "You are an energy usage advisor. Summarize MCP search results concisely. "
        "Highlight anomalous usage, alerts, and suggested next investigative steps."
    )
...
def main() -> None:
    load_dotenv()
    parser = argparse.ArgumentParser(description=f"{INDUSTRY_NAME} Usage Advisor (MCP HTTP)")
