...
# - Synthetic demo sample data (template-driven record count, typically 100K records/industry)
#   - Generated by this repo (not pulled from real customers/employees)
#   - IMPORTANT: Fictitious but contains PII-like fields (e.g., names/emails/phones/addresses; DOB (date of birth) for healthcare/education)
#     Do not treat as "no PII" for compliance/security testing.
...
# Note: `sample_data_size_gb` is a sizing knob for "enterprise demo" scenarios, but the current
# Terraform automation uploads a template-driven number of JSON records to Cosmos/Search and does
# not guarantee an on-disk dataset size (e.g., 50GB).
sample_data_size_gb = 50
